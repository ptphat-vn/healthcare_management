...
export function useChatPeers(searchTerm?: string) {
  ...
  const { data: labUsers, isLoading: isLoadingLab } = useGetAllLabUserQuery(params, { skip: !isPatient } as any);
  const { data: patients, isLoading: isLoadingPat } = useGetAllPatientQuery(params, { skip: !isDoctor } as any);

  return {
    users: isPatient ? labUsers?.data?.user : patients?.data?.user,
    isLoading: isPatient ? isLoadingLab : isLoadingPat,
    targetLabel: isPatient ? "bác sĩ" : "bệnh nhân",
    isPatient,
    isDoctor,
  };
}
